...
class Network(object):
	...
	def create_network_variable(self):
		with tf.variable_scope("EncodingNN") as scope:
			...
			with tf.variable_scope('hidden1') as scope:
				weights = Network.weight_variable([self.encoding_nn_input_size,
	...
	def predict(self, feature_pl, path_pl,logP_pl, sequence_len, flattened_idx_offset):
		with tf.variable_scope("EncodingNN", reuse=True) as scope:
			...
			step_contextual_features = Network.get_contextual_feature(
	...
	def body(sequence_len,
		...
		contextual_features = Network.update_contextual_features(
			contextual_features=contextual_features,
			indices=output_idx,
