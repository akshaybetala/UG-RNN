...
class Molecule:
	...
	def reduce_graph_rings(self):
		...
		while cycle:
			...
			nx.set_node_attributes(self.graph, "atom_features",
								   values={
									   cycle_name: Molecule.atom_features_of_contract_rings(
										   0)})
	...
	def create_feature_vectors(self):
		...
		:return:
		'''
		# create a three dimesnional matrix G,
		# such that Gij is the contextual vector for ith vertex in jth DAG
		self.feature_vector = np.zeros(
		...
		for idx in xrange(self.no_of_atoms):
			...
			for i in xrange(self.no_of_atoms - 1):
				...
				self.feature_vector[idx, i, :] = np.append(
					self.get_atom_features(node1),
					self.get_bond_features(node1, node2))

			self.feature_vector[idx, self.no_of_atoms - 1, :] = np.append(
