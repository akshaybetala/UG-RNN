    ...
    def createPBSDir(self):
        ...
        for f in range(0, self.folds):
            for m in range(0, self.models):
                ...
                if self.train == 0:
                    qsubScript.write(
                        "./ugrnnTrain -d " + self.datasetName + " -t " + self.testID + " -f " + str(f) + " -m " + str(
                            m) + " -vl > ./test/" + self.testID + "/fold" + str(f) + "/model" + str(
                            m) + "/logModel" + str(m))
                else:
                    qsubScript.write(
                        "./ugrnnTrain -d " + self.datasetName + " -t " + self.testID + " -f " + str(f) + " -m " + str(
                            m) + " -tsval > ./test/" + self.testID + "/fold" + str(f) + "/model" + str(
                            m) + "/logModel" + str(m))
                qsubScript.close()
