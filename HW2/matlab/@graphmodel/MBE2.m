...
for i=1:length(order)
  ...
  for pass=1:Nf, 
    ...
    origB{b1} = origB{b1}.*origB{b2}; 
		origB(b2:nUsed-1)=origB(b2+1:nUsed);
    bucket(b2:nUsed-1)=bucket(b2+1:nUsed); bucket{nUsed}=[]; 
		tmp(b2:nUsed-1)=tmp(b2+1:nUsed); 
		score(:,b2:nUsed-1)=score(:,b2+1:nUsed); score(:,nUsed)=-1; score(find(eye(Nf)))=-1; nUsed=nUsed-1;
		for b2=1:nUsed,
			if (b1==b2) continue; end;
      bb1 = min(b1,b2); bb2=max(b1,b2);
      if (length(vunion(variables(bucket{bb1}),variables(bucket{bb2})))<=iBound)
