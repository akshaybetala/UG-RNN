...
The additional matlab interface functions allow
(a) 


 Questions: (a) should we use Matlab's memory allocation for "new" data structures?  (yes?)
            (b) if so, should we differentiate between newly created (or requested), versus auto-created?
            (c) how should destructors work for requested & auto-constructed elements?

			Also, can we create variables without sizes?  What does this mean?  How do we deal with such things later?

			How can we represent Variables value?  (vsize scalar?)  Conversion to vector of individual values?
*/
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  Variables(const vindex Nv, const vindex* vlist=NULL, const vsize* dlist=NULL) {
  ...
  Variables& mxCopy(const mxArray* mNew, vsize* dnew) {
  ...
  Variables& mxSet(mxArray* mNew, const vsize* dnew=NULL) { 
  ...
  Variables& mxNew(const vsize* dnew=NULL) {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  //void setDimsFromGlobal(vindex* dGlobal) { assert(dLocal!=NULL); for (vindex i=0;i<N_;++i) dLocal[i]=dGlobal[v_[i]]; };
  void setDimsFromGlobal(const Variables& Global) { 
    assert(dLocal!=NULL); 
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  Variables  operator& (const Variables& B) const
  {
...
friend std::ostream& operator<< (std::ostream &os, Variables const& v) {
  os<<"{";
  for (vindex i=0;i<v.nvar();i++) os << ( (i) ? "," : "" ) << v[i];
  os<<"}";
	return os;
}
//std::ostream& operator<<(std::ostream& os, const Variables& v) { for (vindex i=0;i<v.N_;i++) os<<v[i]<<" "; }
