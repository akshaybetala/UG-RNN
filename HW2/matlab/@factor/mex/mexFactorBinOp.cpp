...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  if(nlhs > 1)
	...
	if (mxIsNumeric(prhs[0])) F1=mex::Factor(mxGetScalar(prhs[0])); else F1.mxCopy(prhs[0]);
	if (mxIsNumeric(prhs[1])) F2=mex::Factor(mxGetScalar(prhs[1])); else F2.mxSet((mxArray*)prhs[1]);
	F1 BINOP F2;
  plhs[0]=F1.mxGet();						// return the pointer to the new copy

/*
  if (mxIsNumeric(prhs[0])) { 				// if the first argument is a numeric
    F2.mxCopy(prhs[1]); 					// the second must be the factor class variable; copy it
		if (!mxIsEmpty(prhs[0]))
      F1 BINOP mxGetScalar(prhs[0]);			// and act directly
  } else {
    F1.mxCopy(prhs[0]);						// otherwise, it *is* a factor class variable (assumed) so copy it,
